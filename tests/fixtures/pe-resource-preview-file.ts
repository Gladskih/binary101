...
const writeSectionName = (view: DataView, offset: number, name: string): void => {
  for (let index = 0; index < Math.min(name.length, COFF_SHORT_NAME_BYTE_LENGTH); index += 1) {
...
export const createPeResourcePreviewFile = (): MockFile => {
  ...
  view.setUint32(sectionHeaderOffset + 16, FILE_ALIGNMENT, true);
  view.setUint32(sectionHeaderOffset + 20, FILE_ALIGNMENT, true);
  view.setUint32(sectionHeaderOffset + 36, 0x60000020, true);

  const resourceSectionHeaderOffset = sectionHeaderOffset + COFF_SECTION_HEADER_BYTE_LENGTH;
  ...
  bytes[FILE_ALIGNMENT] = 0xc3; // x86-64 `ret`

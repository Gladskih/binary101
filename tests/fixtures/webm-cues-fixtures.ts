...
const createCuesWebmHeader = (): Uint8Array =>
  ebmlElement(
    0x1a45dfa3,
    concatParts([
      ebmlUInt(0x4286, 1, 1),
...
const createCuesWebmInfo = (): Uint8Array => {
...
const createCuesVideoTrack = (): Uint8Array => {
...
const createCuesAudioTrack = (): Uint8Array => {
...
const createCuesWebmTracks = (): Uint8Array =>
  ebmlElement(0x1654ae6b, concatParts([createCuesVideoTrack(), createCuesAudioTrack()]));

const createCuePoint = (timecode: number, track: number, clusterPosition: number): Uint8Array =>
  ebmlElement(
    0xbb,
    concatParts([
...
const createWebmCues = (clusterPosition: number): Uint8Array => {
  const cuesPayload = concatParts([createCuePoint(0, 1, clusterPosition), createCuePoint(1000, 2, clusterPosition)]);
  return ebmlElement(0x1c53bb6b, cuesPayload);
};

const createCuesCluster = (): Uint8Array => {
  const clusterBlocks = concatParts([
    ebmlUInt(0xe7, 0, 1),
    simpleBlock(1, 0, 0x80, new Uint8Array([0x00])),
    simpleBlock(2, 100, 0x00, new Uint8Array([0x00]))
  ]);
  return ebmlElement(0x1f43b675, clusterBlocks);
};

const createCuesSeekHead = (infoOffset: number, tracksOffset: number, cuesOffset: number): Uint8Array => {
...
const createStableCuesMetadata = (info: Uint8Array, tracks: Uint8Array) => {
  let cues = createWebmCues(0);
  let seekHead = createCuesSeekHead(0, 0, 0);
  for (let i = 0; i < 4; i += 1) {
    ...
    const rebuiltCues = createWebmCues(clusterOffset);
    const rebuiltSeek = createCuesSeekHead(infoOffset, tracksOffset, cuesOffset);
    const stable = rebuiltCues.length === cues.length && rebuiltSeek.length === seekHead.length;
    cues = rebuiltCues;
    seekHead = rebuiltSeek;
    if (stable) break;
  }
  return { cues, seekHead };
};

export const createWebmWithCues = () => {
  const ebmlHeader = createCuesWebmHeader();
  const info = createCuesWebmInfo();
  const tracks = createCuesWebmTracks();
  const cluster = createCuesCluster();
  const { cues, seekHead } = createStableCuesMetadata(info, tracks);

...
export const createMp4File = () => {
  ...
  const buildMdia = (handler: string, name: string, timescale: number, duration: number, stbl: Uint8Array) =>
    makeBox("mdia", concatParts([buildMdhd(timescale, duration), buildHdlr(handler, name), buildMinf(handler === "vide", stbl)]));

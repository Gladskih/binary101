...
void test("parseExceptionDirectory parses ARM64 pdata entries that point to xdata records with handlers", async () => {
  ...
  // The xdata header below uses:
  // - X=1 to signal a trailing handler RVA,
  // - E=1 so there is no epilog-scope array,
  // - CodeWords=1 so one 32-bit unwind-code word follows the header.
  dv.setUint32(
    xdataRva,
    1 |

...
void test(
  "analyzePeEntrypointDisassembly follows returns rewritten below saved flags",
  async () => {
    const bytes = new Uint8Array([
      // call 0x140001008; the callee rewrites the saved 0x140001005 return VA.
    ...
    const result = await analyzePeEntrypointDisassembly(
      createFileRangeReader(new MockFile(bytes, "flags-return.exe"), 0, bytes.length),
      {
    ...
    assert.equal(result.blocks.length, 3);
    assert.equal(result.blocks[1]?.startRva, 0x1008);
    assert.equal(
      assertKnownReturnTarget(result.blocks[1]?.instructions.at(-1)?.target).rva,
      0x1011
    );
    assert.equal(result.blocks[2]?.kind, "followed-return");
    assert.equal(result.blocks[2]?.startRva, 0x1011);
    assert.deepEqual(result.blocks[2]?.instructions.map(instruction => instruction.text), [
      "nop",
      "ret"
    ]);

import type { DecodedInstruction, Disassembler } from "llvm-aarch64-disasm";
import {
  classifyAarch64Privilege, type Aarch64SpecialInstruction, type Aarch64InstructionAccess
} from "./special-instructions.js";

type CachedInstruction = { status: "invalid"; specialInstruction?: never } |
  (Pick<DecodedInstruction, "status" | "length" | "features" | "controlFlow" | "target"> &
    { specialInstruction?: Aarch64SpecialInstruction });

export type Aarch64InstructionSample = CachedInstruction & { address: bigint };

// Optional capability keeps the published 0.1.0 package and existing decoder adapters working.
type SampleDisassembler = Disassembler & {
  decodeMetadata?: (bytes: Uint8Array, options: { address: bigint }) => CachedInstruction[];
};

const specialInstructionLabel = (instruction: DecodedInstruction): string => {
  const mnemonic = instruction.mnemonic.toUpperCase();
  const operands = instruction.text.trim().slice(mnemonic.length).trim().toUpperCase().split(/,\s*/);
  if (mnemonic === "MRS") return `${mnemonic} ${operands[1]}`;
  if (["MSR", "DC", "IC", "TLBI", "AT"].includes(mnemonic)) return `${mnemonic} ${operands[0]}`;
  if (mnemonic === "SYS" || mnemonic === "SYSL") {
    return `${mnemonic} ${operands.filter(operand => !/^X(?:\d+|ZR)$/.test(operand)).join(", ")}`;
  }
  return mnemonic;
};

const decodeSample = (
  decoder: SampleDisassembler, bytes: Uint8Array, access: Aarch64InstructionAccess | null
): CachedInstruction => {
  // Format only candidate special instructions, instead of decoding any word twice.
  const instruction = (access ? decoder.decode : decoder.decodeMetadata ?? decoder.decode)
    .call(decoder, bytes.subarray(0, 4), { address: 0n })[0];
  if (!instruction) throw new Error("AArch64 decoder returned no instruction.");
  if (instruction.status === "invalid") return { status: "invalid" };
  return { status: instruction.status, length: instruction.length,
    features: instruction.features, controlFlow: instruction.controlFlow,
    ...(access ? { specialInstruction: {
      instruction: specialInstructionLabel(instruction as DecodedInstruction), access
    } } : {}),
    ...(instruction.target === undefined ? {} : { target: instruction.target }) };
};

// The fixed A64 decoder's feature gates depend on the word, not its PC. Only the branch
// target needs relocating; only a special instruction's label is retained from formatted text.
// https://github.com/Gladskih/llvm-aarch64-disasm/blob/main/src/index.ts
export const createAarch64SampleDecoder = (decoder: SampleDisassembler) => {
  // LM Studio ARM64, first 1M words: 242k misses versus 392k with the old 4096-word
  // flushing cache. Keep only ISA/flow metadata; replace collisions without flushing hot words.
  const words = new Uint32Array(65536);
  const samples = new Array<CachedInstruction | undefined>(65536);
  return (bytes: Uint8Array, address: bigint): Aarch64InstructionSample => {
    if (bytes.length < 4) return { ...decodeSample(decoder, bytes, null), address };
    const word = new DataView(bytes.buffer, bytes.byteOffset, 4).getUint32(0, true);
    // Multiplicative hashing spreads instruction bits across the fixed 16-bit slot index.
    const slot = Math.imul(word, 0x9e3779b1) >>> 16;
    let instruction = words[slot] === word ? samples[slot] : undefined;
    if (!instruction) {
      instruction = decodeSample(decoder, bytes, classifyAarch64Privilege(word));
      words[slot] = word;
      samples[slot] = instruction;
    }
    if (instruction.status === "invalid") return { status: "invalid", address };
    return { ...instruction, address,
      ...(instruction.target === undefined ? {} : {
        target: BigInt.asUintN(64, address + instruction.target)
      }) };
  };
};

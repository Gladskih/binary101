...
const decodeOption = (value, options) => options.find(x => x[0] === value)?.[1] || null;

const decodeFlags = (mask, flags) =>
  flags
    .filter(([bit]) => (mask & bit) !== 0)
    .map(([, name]) => name);

const bigFrom32 = value => BigInt(value >>> 0);

const toSafeNumber = (value, label, issues) => {
...
async function sliceView(file, offset, length) {
...
function parseElfHeader(dv, is64, little, issues) {
  const u16 = offset => dv.getUint16(offset, little);
  const u32 = offset => dv.getUint32(offset, little);
  const u64 = offset => dv.getBigUint64(offset, little);
  const type = u16(0x10);
...
function parseProgramHeader(view, is64, little) {
  const u32 = offset => view.getUint32(offset, little);
  const u64 = offset => view.getBigUint64(offset, little);
  if (is64) {
    const type = u32(0);
    const flags = u32(4);
    return {
...
function parseSectionHeader(view, is64, little) {
  const u32 = offset => view.getUint32(offset, little);
  const u64 = offset => view.getBigUint64(offset, little);
  if (is64) {
    const nameOff = u32(0);
...
async function parseProgramHeaders(file, header, is64, little, issues) {
...
function readStringFromTable(tableDv, offset) {
  if (!tableDv || offset >= tableDv.byteLength) return "";
  return readAsciiString(tableDv, offset, tableDv.byteLength - offset);
}

async function loadSectionNameTable(file, sections, header, issues) {
...
async function parseSectionHeaders(file, header, is64, little, issues) {

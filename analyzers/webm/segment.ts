...
import type { Issues, WebmComputedDuration, WebmComputedTrackDuration, WebmSeekHead, WebmSegment, WebmTrack } from "./types.js";
...
const computeComputedDuration = (
  ...
  for (const trackNumber of trackNumbers) {
    ...
    const durationSeconds = Number.isFinite(endNs) && endNs > 0 ? endNs / 1e9 : 0;
...
export const parseSegment = async (
  ...
  const clusters = await scanClusters(file, segmentHeader, segmentSize, timecodeScaleNs, segment.tracks, issues);

...
import { enrichSegmentWithClusterScan } from "./cluster-analysis.js";
...
export const parseSegmentSummary = async (
  file: File,
  segmentHeader: EbmlElementHeader,
  issues: Issues,
  docTypeLower: string
): Promise<WebmSegment> => {
  return parseSegmentCore(file, segmentHeader, issues, docTypeLower);
};
...
export const parseSegment = async (
  ...
  const segment = await parseSegmentCore(file, segmentHeader, issues, docTypeLower);
  await enrichSegmentWithClusterScan(file, segmentHeader, segment, issues);
  return segment;
};

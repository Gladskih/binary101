...
import type { JpegJfif } from "../../analyzers/jpeg/types.js";
...
function describeDensity(x: number | null, y: number | null, units: number): string {
  if (!x || !y) return "Not specified";
  const unitLabel = describeUnits(units);
  const base = `${x} A- ${y} ${unitLabel}`;
  if (units === 1 || units === 2) {
    if (x === y && x >= 280 && x <= 360) return `${base} - typical print/screen resolution`;
    if (x === y && x <= 120) return `${base} - low resolution (web / preview)`;
    if (x === y && x >= 600) return `${base} - very high resolution (fine print or scanning)`;
  }
  return base;
}

function describeThumbnail(x: number, y: number): string {
  if (!x || !y) return "No embedded thumbnail";
  return `${x} A- ${y} px - small preview image embedded in the file`;
}

export function renderJfifSummary(jfif: JpegJfif): string {
  const out: string[] = [];
  const { versionMajor, versionMinor, units, xDensity, yDensity, xThumbnail, yThumbnail } = jfif;

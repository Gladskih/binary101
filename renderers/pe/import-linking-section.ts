...
export function renderImportLinking(pe: PeParseResult, out: string[]): void {
  ...
  out.push(dd("Validated checks", String(countFindings(linkedModules, "confirmed")), "Cross-checks that matched the PE documentation or another Microsoft-documented layout."));
  out.push(dd("Warnings", String(countFindings(linkedModules, "warning")), "Cross-checks that contradict documented or expected relationships."));
  out.push(dd("Notes", String(countFindings(linkedModules, "info")), "Cross-checks that are informative but not automatically invalid."));

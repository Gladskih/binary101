...
const knownSectionName = name => sectionHintMap[name.toLowerCase()] || null;

function linkerVersionHint(major, minor) {
  ...
  const hint = map[`${major}.0`] || map[version] || (major >= 14 ? "MSVC (VS2015+ era or lld-link)" : "MSVC (pre-VS2015)");
  return `${version} – ${hint}`;
}

function winVersionName(major, minor) {
...
export function renderHeaders(pe, out) {
  ...
  if (pe.dirs?.length) {
    ...
    for (const d of pe.dirs) {
      out.push(dd("[" + d.index + "] " + d.name, `RVA ${hex(d.rva, 8)}, Size ${humanSize(d.size)}`, DD_TIPS[d.name] || "Directory"));
    }
    out.push(`</dl></section>`);
  }

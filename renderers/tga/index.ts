...
const renderTgaHeaderRows = (tga: TgaParseResult, out: string[]): void => {
...
const renderTgaImageFormatRows = (tga: TgaParseResult, out: string[]): void => {
  const header = tga.header;
  const dimensions =
...
const renderTgaDescriptorRows = (tga: TgaParseResult, out: string[]): void => {
  const header = tga.header;
  if (header.imageDescriptor == null) return;
...
const renderTgaImageIdRows = (tga: TgaParseResult, out: string[]): void => {
  if (!tga.imageId) {
    out.push(
      renderDefinitionRow(
        "Image ID",
...
const renderTgaColorMapRows = (tga: TgaParseResult, out: string[]): void => {
  if (!tga.colorMap) return;
...
const renderTgaImageDataRows = (tga: TgaParseResult, out: string[]): void => {
  ...
  if (tga.imageData.expectedDecodedBytes == null) return;
...
const renderTgaFooterRows = (tga: TgaParseResult, out: string[]): void => {
  if (!tga.footer?.present) return;
...
export const renderTga = (tga: TgaParseResult | null): string => {
  ...
  out.push("<dl>");
  renderTgaHeaderRows(tga, out);
  renderTgaImageFormatRows(tga, out);
  renderTgaDescriptorRows(tga, out);
  renderTgaImageIdRows(tga, out);
  renderTgaColorMapRows(tga, out);
  renderTgaImageDataRows(tga, out);
  renderTgaFooterRows(tga, out);
  out.push("</dl>");

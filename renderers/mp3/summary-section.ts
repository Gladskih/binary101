...
import type { Mp3SuccessResult, Mp3Summary } from "../../analyzers/mp3/types.js";

const renderCodecRows = (summary: Mp3Summary): string[] => {
...
const renderTimingRows = (summary: Mp3Summary, audioDataBytes: number | undefined): string[] => [
  renderDefinitionRow(
    "Sample rate",
    withFieldNote(
      valueWithHint(
...
const renderTagRows = (summary: Mp3Summary): string[] => [
  renderDefinitionRow(
    "ID3v2 tag",
    withFieldNote(
...
export function renderSummary(mp3: Mp3SuccessResult | null | unknown): string {
  ...
  return (
    "<h4>Summary</h4><dl>" +
    [
      ...renderCodecRows(summary),
      ...renderTimingRows(summary, audioDataBytes),
      ...renderTagRows(summary)
    ].join("") +
    "</dl>"
  );
}
